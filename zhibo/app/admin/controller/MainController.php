...
class MainController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        //消息记录数
        $messageCount = Db::name('portal_message')->count();
        //注册会员人数
        $memberCount = Db::name('member')->where('adminid','<>', 14)->count();
        ...
        return $this->fetch('index_tj');
    }

    public function ajax_date_msg()
    {
        $portalMessageModel = new PortalMessageModel();
        $list = $portalMessageModel->getMessageReport();
    ...
    public function onlineUserCount()
    {
        ...
        $onlineLogModel = new OnlineLogModel();
        $onlineLog = $onlineLogModel->getOnlineReport();
        $dayHours = $onlineLogModel->getDayHours();
        $this->success('返回成功', '', array('categories' => $dayHours, 'series' => $onlineLog));
    }
