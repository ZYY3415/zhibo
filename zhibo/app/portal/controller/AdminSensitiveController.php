...
class AdminSensitiveController extends AdminBaseController
{
    ...
    function msgRid($obj,$param)
    {
        ...
        $_SESSION['admin_sensitive'] = serialize($obj['messageId']->all());
    ...
    public function delete()
    {
        ...
        if (isset($param['id'])) {
            ...
            $result       = $ChatModel->where(['id' => $id])->delete();            
            if (!$result) {
                $this->error('删除失败',"");
            }
                $this->success("删除成功！", '');
        ...
        if (isset($param['ids'])) {
            ...
            $result  = $ChatModel->where(['id' => ['in', $ids]])->delete();
            if (!$result) {
                $this->error('删除失败',"");
            }
                $this->success("删除成功！", '');    
    ...
    {
        if(!isset($_SESSION['admin_sensitive'])){
            return 'error';
        }
        $obj = unserialize($_SESSION['admin_sensitive']);
        ...
        $field_width = ['C'=>20,'E'=>20,'F'=>20,'H'=>20];
