...
class Tree
{
    ...
    public $role = 0;
    public function init($arr = [],$role=0)
    {
        $this->arr = $arr;
        $this->ret = '';
        $this->role = $role;
     ...
     * return  array  返回同层
    **/
    public function getTreeTdArray($myId,$level = 0,$flag = false)
    {
        if(is_array($this->arr))
        {
            $role = $this->role;
            ...
            switch($role)
            {
                case 1:
                    ...
                    foreach($this->arr as $key=>$value)
                    {
                        if($value['parent_id'] == $myId)
                        {
                            if($flag)
                            {
                                $this->returnArray[] = $value['id'];
                                $this->getTreeTdArray($value['id'],$level,$flag);
                            }else{
                                $value['level'] = $level;
                                $this->returnArray[] = $value;
                                $this->getTreeTdArray($value['id'],$level);
                            }
                        }
                    }
                break;
                default:
                    foreach($this->arr as $key=>$value)
                    {
                        if($level === 1)
                        {
                            if($value['parent_id'] == $myId && $value['id'] == $role)
                            {
                                if($flag)
                                {
                                    $this->returnArray[] = $value['id'];
